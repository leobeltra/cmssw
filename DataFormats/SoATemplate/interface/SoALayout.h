...
#define _COUNT_EIGEN_COLS_IMPL(VALUE_TYPE, CPP_TYPE, NAME, ARGS)                                                          \
  _SWITCH_ON_TYPE(VALUE_TYPE,                                                                                          \
      /* Scalar */                                                                                                     \
      ,                                                                                                              \
      /* Column */                                                                                                     \
...
#define _COUNT_EIGEN_COLS(R, DATA, TYPE_NAME) BOOST_PP_EXPAND(_COUNT_EIGEN_COLS_IMPL TYPE_NAME)

#define _GENERATE_EIGEN_COLS_INDEXES_IMPL(VALUE_TYPE, CPP_TYPE, NAME, ARGS)                                                  \
  _SWITCH_ON_TYPE(VALUE_TYPE,                                                                                          \
      /* Scalar */                                                                                                     \
      currentColIndex ++;                                                                                                 \
      ,                                                                                                                \
      ...
      ,                                                                                                                \
      /* Eigen column */                                                                                               \
      eigenIndexes[currentEigenColIndex].first = currentColIndex;                                                      \
      eigenIndexes[currentEigenColIndex].second = CPP_TYPE::RowsAtCompileTime * CPP_TYPE::ColsAtCompileTime;           \
      currentColIndex ++;                                                                                               \
      currentEigenColIndex ++;                                                                                         \
  )

#define _GENERATE_EIGEN_COLS_INDEXES(R, DATA, TYPE_NAME) BOOST_PP_EXPAND(_GENERATE_EIGEN_COLS_INDEXES_IMPL TYPE_NAME)
    ...
    struct ConstDescriptor {                                                                                                \
      ConstDescriptor(ConstView const& view)                                                                                           \
          : buff{ _ITERATE_ON_ALL_COMMA(_ASSIGN_SPAN_TO_COLUMNS, ~, __VA_ARGS__)} {}                                   \
                                                                                                                      \
          static constexpr size_type numberOfEigenCols() {                                                                \
            size_type eigenCols = 0;                                                                                       \
            _ITERATE_ON_ALL(_COUNT_EIGEN_COLS, ~, __VA_ARGS__)                                                            \
            return eigenCols;                                                                                            \
          }                                                                                                                \
                                                                                                                          \
          static constexpr std::array<std::pair<size_type, size_type>, numberOfEigenCols()> indexesOfEigenCols() {                               \
            [[maybe_unused]] size_type currentColIndex = 0;                                                                 \
            [[maybe_unused]] size_type currentEigenColIndex = 0;                                                          \
            std::array<std::pair<size_type, size_type>, numberOfEigenCols()> eigenIndexes;                                                        \
            _ITERATE_ON_ALL(_GENERATE_EIGEN_COLS_INDEXES, ~, __VA_ARGS__)                                                        \
            return eigenIndexes;                                                                                          \
          }                                                                                                                 \
                                                                                                                          \
          ...
          static constexpr size_type eigenColsCount = numberOfEigenCols();                                                \
          static constexpr std::array<std::pair<size_type, size_type>, eigenColsCount> eigenIndexes = indexesOfEigenCols();                 \
      };                                                                                                                \
                                                                                                                      \
    struct Descriptor {                                                                                                \
