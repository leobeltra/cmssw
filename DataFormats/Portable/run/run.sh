...
CMSSW_BASE="/shared/CMSSW_16_0_0_pre1"
SRC_DIR="$CMSSW_BASE/src/DataFormats/Portable"
RUN_DIR="$CMSSW_BASE/test/el9_amd64_gcc13"
OUT_DIR="$SRC_DIR/results"
OUT_CSV="$OUT_DIR/results_avg_cpu.csv"
...
EXEC="./GenericViewSerialSync"
[[ -x "$EXEC" ]] || { echo "ERROR: $EXEC not found or not executable"; exit 1; }

# CSV header
echo "element_size,mean_soas,std_soas,mean_genericview,std_genericview" > "$OUT_CSV"
...
for size in "${SIZE_LIST[@]}"; do
  ...
  for ((r=1; r<=TOTAL_RUNS; ++r)); do
    ...
    t_soa="$(grep -m1 -E 'Total execution time copying the data:' <<<"$out" | awk '{print $(NF-1)}')"
    t_blk="$(grep -m1 -E 'Total execution time using deepCopy:' <<<"$out" | awk '{print $(NF-1)}')"
