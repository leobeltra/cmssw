...
struct FillSoA
{
  ...
  ALPAKA_FN_ACC void operator()(TAcc const &acc, PositionView positionView, PCAView pcaView) const {
    ...
    for (auto local_idx : cms::alpakatools::uniform_elements(acc, positionView.metadata().size())) {
      ...
      pcaView[local_idx].eigenvector_1() = positionView[local_idx].x() / TIME;
      pcaView[local_idx].eigenvector_2() = positionView[local_idx].y() / TIME;
      pcaView[local_idx].eigenvector_3() = positionView[local_idx].z() / TIME;
  ...
  for (auto const& device : cms::alpakatools::devices<Platform>()) {
    ...
    SECTION("Deep copy the ConstView host to host and device to device") {
      ...
      for (size_t i = 0; i < elems; i++) {
        ...
        REQUIRE(genericViewHostCollection[i].candidateDirection()(0) == pcaViewHostCollection[i].candidateDirection()(0));
        REQUIRE(genericViewHostCollection[i].candidateDirection()(1) == pcaViewHostCollection[i].candidateDirection()(1));
        REQUIRE(genericViewHostCollection[i].candidateDirection()(2) == pcaViewHostCollection[i].candidateDirection()(2));
      }
    }
