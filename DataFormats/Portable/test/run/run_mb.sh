...
OUT_CSV="$OUT_DIR/results_mem_cuda.csv"
...
for size in "${SIZE_LIST[@]}"; do
  ...
  for ((r=1; r<=TOTAL_RUNS; ++r)); do
    ...
    t_soa="$(grep -m1 -E 'SoA time:' <<<"$out" | awk '{print $(NF-1)}' || true)"
    t_aos="$(grep -m1 -E 'AoS time:' <<<"$out" | awk '{print $(NF-1)}' || true)"
