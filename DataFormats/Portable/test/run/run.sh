...
EXEC="./Device_spansCudaAsync"
[[ -x "$EXEC" ]] || { echo "ERROR: $EXEC not found or not executable"; exit 1; }

# CSV header
echo "element_size,mean_pointers,std_pointers,mean_spans,std_spans" > "$OUT_CSV"
...
for size in "${SIZE_LIST[@]}"; do
  ...
  mean_pointers="$(awk '{print $1}' <<<"$soa_stats")"
  std_pointers="$(awk '{print $2}' <<<"$soa_stats")"
  mean_spans="$(awk '{print $1}' <<<"$blk_stats")"
  std_spans="$(awk '{print $2}' <<<"$blk_stats")"

  echo "$size,$mean_pointers,$std_pointers,$mean_spans,$std_spans" >> "$OUT_CSV"
