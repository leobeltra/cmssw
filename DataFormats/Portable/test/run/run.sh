...
EXEC="./PortableAoSCudaAsync"
[[ -x "$EXEC" ]] || { echo "ERROR: $EXEC not found or not executable"; exit 1; }

# CSV header
echo "element_size,mean_soa,std_soa,mean_aos,std_aos" > "$OUT_CSV"
...
for size in "${SIZE_LIST[@]}"; do
  ...
  for ((r=1; r<=TOTAL_RUNS; ++r)); do
    ...
    t_soa="$(grep -m1 -E 'Total execution time for soa:' <<<"$out" | awk '{print $(NF-1)}' || true)"
    t_aos="$(grep -m1 -E 'Total execution time for aos:' <<<"$out" | awk '{print $(NF-1)}' || true)"

    # Sanity check
    if [[ -z "${t_soa:-}" || -z "${t_aos:-}" ]]; then
  ...
  aos_stats="$(tail -n +$((WARMUP+1)) "$tmp_aos" | awk "$awk_mean_std")"

  mean_soa="$(awk '{print $1}' <<<"$soa_stats")"
  std_soa="$(awk '{print $2}' <<<"$soa_stats")"
  mean_aos="$(awk '{print $1}' <<<"$aos_stats")"
  std_aos="$(awk '{print $2}' <<<"$aos_stats")"

  echo "$size,$mean_soa,$std_soa,$mean_aos,$std_aos" >> "$OUT_CSV"

  rm -f "$tmp_soa" "$tmp_aos"
done
